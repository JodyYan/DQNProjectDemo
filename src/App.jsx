...
function App() {
  ...
  const { allocationWeights, estReturn, maxDrawdown } = useMemo(() => {
    ...
    const cashWeight =
      marketState === "bear" ? baseCashWeight + 20 : baseCashWeight;
    ...
    if (n === 0) {
      const cashAmount = principal.toLocaleString("zh-TW", {
        style: "currency",
        currency: "TWD",
        maximumFractionDigits: 0,
      });
      weights.push({
        label: `Cash 現金 (100%) : ${cashAmount}`,
        value: 100,
        color: "#64748b",
        id: "cash",
      });
      ...
      weights = stockWeights.map((sw, i) => {
        ...
        const stockName =
          cacheData?.name && cacheData.name !== sw.label ? cacheData.name : "";
        const displayName = stockName ? `${sw.label} ${stockName}` : sw.label;
        const amount = ((principal * sw.value) / 100).toLocaleString("zh-TW", {
          style: "currency",
          currency: "TWD",
          maximumFractionDigits: 0,
        });
      ...
      if (cashWeight > 0) {
        const cashAmount = ((principal * cashWeight) / 100).toLocaleString(
          "zh-TW",
          { style: "currency", currency: "TWD", maximumFractionDigits: 0 },
        );
        weights.push({
          id: "cash",
          label: `Cash 現金 (${cashWeight.toFixed(0)}%) : ${cashAmount}`,
          value: cashWeight,
          color: "#64748b",
        });
      }
  ...
  const marketStateLabel = { bull: "多頭", neutral: "震盪", bear: "空頭" }[
    marketState
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#1e293b] p-6 rounded-xl shadow-lg border border-slate-700/50">
              <h2 className="text-xl font-bold mb-8 text-slate-100">
                1. AI 強化學習投資組合配置
              </h2>
            ...
            <div className="bg-[#1e293b] p-6 rounded-xl shadow-lg border border-slate-700/50 space-y-6">
              ...
              {selectedStock && (
                <PricingChart
                  ...
                  buyPrice={
                    (stockDataCache[selectedStock]?.price || 100) *
                    (1 - 0.01 * (11 - riskTolerance))
                  }
                  sellPrice={
                    (stockDataCache[selectedStock]?.price || 100) *
                    (1 + 0.01 * (5 + riskTolerance))
                  }
                />
              )}
              ...
              <div className="flex gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (exactCode) {
                        ...
                        const match = Object.entries(stockDataCache).find(
                          ([code, data]) => data.name.includes(val),
                        );
                        if (match) {
                ...
                <button
                  onClick={async () => {
                    ...
                    const matchByName = Object.entries(stockDataCache).find(
                      ([code, data]) =>
                        data.name === targetCode ||
                        data.name.includes(targetCode),
                    );
                    ...
                    try {
                      ...
                      if (!res.ok) {
                        alert(
                          `網路連線異常，無法連線至證交所伺服器 (HTTP ${res.status})。`,
                        );
                      ...
                      let found = false;
                      if (data.msgArray && data.msgArray.length > 0) {
                        const item = data.msgArray.find(
                          (i) => i.c === targetCode,
                        );
                        if (item) {
                          const priceStr =
                            item.z && item.z !== "-" ? item.z : item.y;
                          const price = parseFloat(priceStr);
                          if (!isNaN(price)) {
                            setStockDataCache((prev) => ({
                              ...prev,
                      ...
                      if (found) {
                        ...
                        if (isNaN(Number(targetCode))) {
                          alert(
                            `證交所回報：找不到名稱或代號為「${searchQuery}」的即時行情。\n\n原因可能為：\n1. 該公司未公開發行、非上市/上櫃公司。\n2. 名稱輸入錯誤 (請嘗試輸入完整公司名稱或直接輸入代號)。\n3. 該標的已下市。`,
                          );
                        } else {
                          alert(
                            `證交所回報：找不到代號為「${targetCode}」的即時行情。\n\n原因可能為：\n1. 該代號不存在或已下市。\n2. 該代號為興櫃股票 (本系統目前支援上市/上櫃股票與ETF)。\n3. 系統連線延遲，請稍後再試。`,
                          );
                        }
                      }
                    } catch (err) {
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {customStocks.map((stock) => {
                  ...
                  const stockName =
                    cacheData?.name && cacheData.name !== stock
                      ? cacheData.name
                      : "";
                  const displayName = stockName
                    ? `${stock} ${stockName}`
                    : stock;
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        onClick={(e) => {
                          e.stopPropagation();
                          const newStocks = customStocks.filter(
                            (s) => s !== stock,
                          );
                          setCustomStocks(newStocks);
                          if (selectedStock === stock)
                            setSelectedStock(
                              newStocks.length > 0 ? newStocks[0] : null,
                            );
                        }}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#1e293b] p-6 rounded-xl shadow-lg border border-slate-700/50 space-y-4">
              ...
              <div className="space-y-4 bg-slate-800/80 p-5 rounded-lg text-sm text-slate-300 font-mono leading-relaxed border-l-4 border-blue-500/50">
                <p>
                  <span className="text-blue-400 font-bold mb-1 block">
                    1. DQN 獎勵函數映射說明
                  </span>
                  目前 AI 代理人 (Agent) 設定之風險懲罰係數 λ = {riskTolerance}
                  。<br />
                  優化目標為最大化獎勵函數 R = μ - λσ²。
                </p>
                <p>
                  ...
                  {marketState === "bull" &&
                    `預期報酬 μ = 6% + (風險 ${riskTolerance} × 1.2)% = ${estReturn}%`}
                  {marketState === "bear" &&
                    `預期報酬 μ = 2% - (風險 ${riskTolerance} × 1.5)% = ${estReturn}%`}
                  {marketState === "neutral" &&
                    `預期報酬 μ = 4% + (風險 ${riskTolerance} × 0.1)% = ${estReturn}%`}
                </p>
                <p>
                  <span className="text-blue-400 font-bold mb-1 block">
                    3. 資產配置決策說明
                  ...
                  {marketState === "bull" &&
                    `狀態評估為 Bull (多頭)。為最大化 Q-Value，策略網路 (Policy Network) 降低現金權重至 ${allocationWeights.find((w) => w.id === "cash")?.value.toFixed(0) || 0}%，並將高 Beta 成長股權重放大至 ${allocationWeights.find((w) => w.id !== "cash")?.value.toFixed(0) || 0}%。`}
                  {marketState === "bear" &&
                    `狀態評估為 Bear (空頭)。偵測到下行風險增加，觸發避險機制。現金與無風險資產權重提升至 ${allocationWeights.find((w) => w.id === "cash")?.value.toFixed(0) || 0}%，並重倉防禦型資產至 ${customStocks.length > 0 ? (allocationWeights.find((w) => w.id === customStocks[customStocks.length - 1])?.value || 0).toFixed(0) : 0}% 以降低投資組合波動率 (σ²)。`}
                  {marketState === "neutral" &&
                    `狀態評估為 Neutral (震盪)。維持平衡配置，現金佔比 ${allocationWeights.find((w) => w.id === "cash")?.value.toFixed(0) || 0}%，平均分散風險。`}
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#1e293b] p-6 rounded-xl shadow-lg border border-slate-700/50 space-y-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center gap-4 bg-[#0f172a] border border-slate-600 rounded-lg px-4 py-2">
                  ...
                  />
                  <span className="text-xs font-bold text-slate-300 w-8 whitespace-nowrap">
                    Lv {riskTolerance}
                  </span>
                </div>
              </div>
                ...
                <PricingCard
                  ...
                  stockData={{
                    regularMarketPrice:
                      stockDataCache[selectedStock]?.price || 100,
                    shortName:
                      stockDataCache[selectedStock]?.name &&
                      stockDataCache[selectedStock].name !== selectedStock
                        ? stockDataCache[selectedStock].name
                        : "(無收盤報價)",
                  }}
